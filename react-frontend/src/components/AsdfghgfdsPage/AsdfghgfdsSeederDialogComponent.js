...
import seederData from "./asdfghgfds.seeder.json";
...
const AsdfghgfdsSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("asdfghgfds").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(AsdfghgfdsSeederDialogComponent);
